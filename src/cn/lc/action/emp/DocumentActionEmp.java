...
@ParentPackage("root")
@Namespace(value="/")
@Results(value = {@Result(name = "document.list",location = "/pages/jsp/emp/emp_document_list.jsp"),
        @Result(name = "document.listByUser",location = "/pages/jsp/emp/emp_document_listByUser.jsp"),
        @Result(name = "document.insert",location = "/pages/jsp/emp/emp_document_insert.jsp"),
        @Result(name = "document.update",location = "/pages/jsp/emp/emp_document_update.jsp"),
        @Result(name = "updateVF",location = "/DocumentActionEmp!list.action",type = "redirectAction"),
        @Result(name = "insertVF",location = "/DocumentActionEmp!insertPre.action",type = "redirectAction")})
...
public class DocumentActionEmp extends AbstractAction {
    ...
    public String insert(){
        User admin = (User) super.getSession().getAttribute("emp");
            ...
            if (this.documentService.insert(document)) {
                ...
                if (this.file != null){
                    if (super.saveSingleFile(filePath, this.file)){
                        super.setMsgAndUrl("insert.success.msg", "emp_document.insertPre.action");
                    }else {
                        super.setMsgAndUrl("insert.failure.msg", "emp_document.insertPre.action");
                    }
                }
                super.setMsgAndUrl("insert.success.msg", "emp_document.insertPre.action");
            }else {
    ...
    public String delete(){
        ...
        try {
            if (this.documentService.delete(set,emp.getUserid())) {
                if (files.size()>0){
                    String filePath = super.getApplication().getRealPath("/upload/document/") + document.getFile();
                    deleteFileBatch(filePath,files);
                }
                super.setMsgAndUrl("delete.success.msg", "emp_document.list.action");
            } else {
                super.setMsgAndUrl("delete.failure.msg", "emp_document.list.action");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String update(){
        ...
        try {
            document.setDoctype(doctype);
            if (this.documentService.update(document,emp.getUserid())) {
                String filePath = super.getApplication().getRealPath("/upload/document/") + document.getFile();
                if (this.file != null){
                    if (super.saveSingleFile(filePath, this.file)){
                        super.setMsgAndUrl("update.success.msg", "emp_document.list.action");
                    }else {
                        super.setMsgAndUrl("update.failure.msg", "emp_document.list.action");
                    }
                }
                super.setMsgAndUrl("update.success.msg", "emp_document.list.action");
            }else {
    ...
    public String list(){
        try{
            ...
            super.handleSplit(map.get("documentCount"),"emp.role.split.url",null,null);
    ...
    public String listByUser(){
        ...
        try{
            Map<String, Object> map = this.documentService.listByUser(emp.getUserid(),super.getCp(),super.getLs(),super.getCol(),super.getKw());
            super.handleSplit(map.get("documentCount"),"emp.role.split.url",null,null);
            super.getRequest().setAttribute("all",map.get("allDocuments"));
            super.getRequest().setAttribute("allDoctypes",map.get("allDoctypes"));
        }catch (Exception e){
