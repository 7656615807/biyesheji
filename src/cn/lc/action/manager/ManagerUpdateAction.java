...
@ParentPackage("root")
@Namespace("/")
@Results(value = {@Result(name = "updatePasswordVF",location = "/pages/jsp/manager/manager_password_edit.jsp"),
                    @Result(name = "manager.updatepre",location = "/pages/jsp/manager/manager_update.jsp")})
@Action("ManagerUpdateAction")
public class ManagerUpdateAction extends AbstractAction {
    ...
    public String update(){
        ...
        try {
            if (this.userServiceCommon.update(user)) {
                ...
                super.setMsgAndUrl("update.failure.msg","manager.user.update.action");
            }
        }catch (Exception e){
            super.setMsgAndUrl("update.failure.msg","manager.user.update.action");
            e.printStackTrace();
