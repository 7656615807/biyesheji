$(function() {
    ...
    $("button[id*='adminBtn-']").each(function () {
        ...
        $(this).on("click",function () {
            $.post("DocumentActionManager!updatePre.action",{"document.did":did},
                function (data) {
                console.log(data);
    ...
    $("#submitBtn").on("click",function() {
        ...
        alert(formdata);
        $.post("DocumentActionManager!update.action",{"document.title":title,"document.did":did,"document.content":content,"oldFile":oldFile,"file":formdata,"doctype.dtid":dtid},
            function (data) {
                $("#title-"+ did).text(name);
    ...
    $("#delBtn").on("click",function(){
        ...
        if(data == ""){ //现在没有选择要删除的内容
            ...
            if(window.confirm("你确定删除这些文档吗？")){
                var url = "/DocumentActionManager!delete.action?ids="+data;
                window.location = url
            }
        }
    });
