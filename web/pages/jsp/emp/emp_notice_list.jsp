...
<head>
    ...
    <script type="text/javascript" src="js/emp/emp_notice_update.js"></script>
                        ...
                        <c:if test="${all != null}">
                            <c:forEach items="${all}" var="a">
                                <tr>
                                    ...
                                    <td class="text-center"><a href="/UserActionEmp!show.action?user.userid=${a.user.userid}"><span id="name-${a.snid}">${a.user.name}</span></a></td>
                    ...
                    <div id="pageDiv" class="text-right">
                        <c:if test="${allRecorders != 0}">
                            <ul class="pagination pagination-sm" id="pagecontrol">
                                ...
                                <c:if test="${currentPage != 1}">
                                    <li><a style='cursor:pointer;' href="NoticeActionEmp!list.action?cp=1&col=${column}&kw=${keyword}">首页</a></li>
                                    <li><a style='cursor:pointer;' href="NoticeActionEmp!list.action?cp=${currentPage - 1}&col=${column}&kw=${keyword}">上一页</a></li>
                                </c:if>
                                ...
                                <c:if test="${currentPage != allRecorders}">
                                    <li><a style='cursor:pointer;'  href="NoticeActionEmp!list.action?cp=${currentPage + 1}&col=${column}&kw=${keyword}">下一页</a></li>
                                    <li><a style='cursor:pointer;'  href="NoticeActionEmp!list.action?cp=${allRecorders}&col=${column}&kw=${keyword}">尾页</a></li>
                                </c:if>
                                <p>
